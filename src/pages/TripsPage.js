...
export default function TripsPage() {
  ...
  const fetchTrips = async () => {
    try {
      const { data } = await axios.get('http://3.239.229.120:5000/api/rides/all', {
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { _id, user, fare, otp, payment, pickup, destination, status, driver } = row;
                      ...
                      <>
                        <TableRow hover key={_id} tabIndex={-1} role="checkbox" selected={selectedUser}>
                          ...
                          <TableCell align="left">
                            {pickup?.pickUpAddress.slice(0, 20) || pickup?.address?.slice(0, 20)}...
                          </TableCell>
                          <TableCell align="left">
                            {destination?.destinationAddress.slice(0, 20) || destination?.address?.slice(0, 20)}...
                          </TableCell>
                          <TableCell align="left">$ {fare}</TableCell>
                          <TableCell align="left">{payment ? 'True' : 'False'}</TableCell>
                          <TableCell align="left">{status}</TableCell>

                          <TableCell align="left">
                            <div
                              style={{ margin: '0rem 1rem', width: '1.7rem', color: 'blue', cursor: 'pointer' }}
                              onClick={() => navigate(`/dashboard/trip/view/${_id}`)}
                            >
                              <svg
