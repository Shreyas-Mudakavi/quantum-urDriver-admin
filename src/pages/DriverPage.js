...
function applySortRoleFilter(array, comparator, query) {
  ...
  if (query) {
    return filter(array, (_user) => _user?.role?.indexOf(query.toLowerCase()) !== -1);
  }
  return stabilizedThis.map((el) => el[0]);
}
...
export default function DriverPage() {
  ...
  const fetchUsers = async () => {
    try {
      const { data } = await axios.get('http://3.239.229.120:5000/api/admin/drivers', {
  ...
  const deleteUser = async (id) => {
    try {
      const { data } = await axios.delete(`http://localhost:5000/api/admin/delete-user/${id}`, {
        headers: { Authorization: token },
      });
  ...
  const filteredRoleUsers =
    filterRole !== '' && applySortRoleFilter(usersList, getComparator(order, orderBy), filterRole);

  const isNotFound = (!filteredUsers.length && !!filterName) || (!filteredRoleUsers?.length && !!filterRole);
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers
                        ...
                        return (
                          <>
                            <TableRow hover key={_id} tabIndex={-1} role="checkbox" selected={selectedUser}>
                              ...
                              <TableCell align="left">{account_type}</TableCell>
                              <TableCell align="left">{email}</TableCell>
                              <TableCell align="left">{city}</TableCell>
                              <TableCell align="left">{license?.license_no}</TableCell>
                              <TableCell align="left">{phone}</TableCell>
                              <TableCell align="left">{sex}</TableCell>
                              <TableCell align="left">{age}</TableCell>
                              <TableCell align="left">{moment(createdAt).utc().format('MMMM DD, YYYY')}</TableCell>
                              ...
                              <TableCell align="left">
                                <div style={{ display: 'flex' }}>
                                  <div
                                    ...
                                    onClick={() => navigate(`/dashboard/driver/view/${_id}`)}
                                  >
                                    <svg
                    ...
                    : filteredRoleUsers &&
                      filteredRoleUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                        const { _id, firstname, lastname, role, status, company, profilePic, verified, city, mobile } =
                          row;
                        const selectedUser = selected.indexOf(_id) !== -1;
                            ...
                            <TableRow hover key={_id} tabIndex={-1} role="checkbox" selected={selectedUser}>
                              ...
                              <TableCell align="left">
                                <div style={{ display: 'flex' }}>
                                  <div
                                    style={{ margin: '0rem 1rem', width: '1.7rem', color: 'blue', cursor: 'pointer' }}
                                    onClick={() => navigate(`/dashboard/user/view/${_id}`)}
                                  >
                                    <svg
                                      xmlns="http://www.w3.org/2000/svg"
                                      fill="none"
                      ...
                      <TableCell align="center" colSpan={6} sx={{ py: 3 }}>
                        ...
                        >
                          ...
                          <Typography variant="body2">
                            ...
                            <strong>&quot;{filterName || filterRole}&quot;</strong>.
